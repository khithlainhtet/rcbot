...
class TelegramCheckerService:
    ...
    async def _check_phone_with_agent(self, phone: str, agent) -> Dict[str, Any]:
        ...
        if reg_state == "unregistered":
            return {"state": "unregistered", "error": "Phone number is not registered on Telegram", "phone": phone}

        if reg_state == "error":
            return {"state": "error", "error": "Rate limit or API error occurred. Try again later.", "phone": phone}

        if reg_state != "registered":
            return {"state": "error", "error": f"Unexpected registration state: {reg_state}", "phone": phone}
